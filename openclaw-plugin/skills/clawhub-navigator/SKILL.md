...
description: OpenClaw 生态导航助手。帮助用户查找项目、技能、教程、博客和社区动态，回答关于 OpenClaw 的一切问题。支持 cron 定时自动同步博客。
version: 1.1.0
author: js-clawhub
---
...
| cron 隔离会话触发（消息包含"博客自动同步"） | 执行 **定时同步流程** |
...
本技能支持通过 cron 定时任务自动同步博客。如果用户希望自动化博客导入和翻译，应提醒用户执行以下命令一键配置：
...
- `sources.json` 中的源路径在运行环境中可访问。
- LLM API 已在插件设置或 `.env` 中正确配置（`CLAWHUB_API_BASE_URL`、`CLAWHUB_API_KEY`、`CLAWHUB_API_MODEL`）。
