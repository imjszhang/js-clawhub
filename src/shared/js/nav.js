...
const Nav = {
    ...
    items: [
        { i18nKey: 'nav.projects', href: '/' },
        { i18nKey: 'nav.skills', href: '/skills/' },
    ...
    /**
     * Get label for a nav item (i18n-aware)
     */
    _label(item) {
        if (typeof I18nManager !== 'undefined') {
            return I18nManager.t(item.i18nKey);
        }
        // Fallback: extract last part of key
        return item.i18nKey.split('.').pop().toUpperCase();
    },
    ...
    render(targetId) {
        ...
        const navItemsHTML = this.items.map(item => {
            ...
            return `<a href="${resolvedHref}" class="hover:underline decoration-2 underline-offset-4 ${activeClass}">${this._label(item)}</a>`;
        }).join('');

        // Language toggle button
        const currentLocale = typeof I18nManager !== 'undefined' ? I18nManager.getLocale() : 'zh-CN';
        const toggleLabel = currentLocale === 'zh-CN' ? 'EN' : '中';
