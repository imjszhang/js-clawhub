...
const Pulse = {
        ...
        return grouped.map(([date, items]) => `
            <div class="mb-10">
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <span class="font-mono text-sm font-bold text-black/50">${items.length} items</span>
                </div>
    ...
    renderEngagement(engagement) {
        ...
        if (engagement.likes != null) {
            metrics.push(`<span class="flex items-center gap-1" title="Likes">
                ...
                ${this.formatNumber(engagement.likes)}</span>`);
        }
        if (engagement.replies != null) {
            metrics.push(`<span class="flex items-center gap-1" title="Replies">
                ...
                ${this.formatNumber(engagement.replies)}</span>`);
        }
        if (engagement.retweets != null) {
            metrics.push(`<span class="flex items-center gap-1" title="Retweets">
                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z"/></svg>
                ${this.formatNumber(engagement.retweets)}</span>`);
        }
